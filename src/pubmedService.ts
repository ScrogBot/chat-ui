...
const fetchArticles = async (querykey: string, webenv: string): Promise<PubMedArticle[]> => {
  try {
    const response = await fetch(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&query_key=${querykey}&WebEnv=${webenv}&retmode=xml`);
    ...
    const articles = data.PubmedArticleSet.PubmedArticle.map((article: any) => ({
...
export const performPubMedSearch = async (query: string): Promise<PubMedSearchResponse> => {
  try {
    const response = await fetch(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${query}&usehistory=y&retmode=json&retmax=20&rettype=abstract`);
...
export const performPubMedFetch = async (querykey: string, webenv: string): Promise<PubMedFetchResponse> => {
  try {
